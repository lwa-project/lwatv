Setting Up the LWATV GUI on Raspberry Pi OS
===========================================

As of the Tkinter port the default `lwaTV3.py` runs on the Raspberry Pi
without modification, so there is no longer a Pi-specific build of the
script.  These instructions cover Raspberry Pi OS Bookworm and Trixie.
...
-----------
 ...
 3. Run the provisioning script:
    ```
    bash ~/LWATV/RaspberryPi/setup/setup.sh
    ```
    It installs the required packages, populates the movie cache, and
    installs the desktop autostart entry and the daily movie-update cron
    job.  It is safe to re-run later to update the machine.
...
Manual Setup
------------
If you would rather understand the moving parts (or the script does not fit
...
Video Sink
----------
The Pi uses playbin's default video sink, which embeds correctly into the
GUI window.  If you ever land on a display stack where the movie does not
appear inside the window, force an X11 sink via the `LWATV_VIDEO_SINK`
